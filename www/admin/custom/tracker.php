...
$d = str_replace('/admin/custom/tracker.php','',$f);
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/custom/tracker.php?a=view')"><i class="icon-list-alt"></i><?=lg('View')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/custom/tracker.php?a=edit')"><i class="icon-edit"></i><?=lg('Edit')?></button>
    </div>
  </div>
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/custom/tracker.php?a=view')"><i class="icon-list-alt"></i><?=lg('View')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/custom/tracker.php?a=view')"><i class="icon-reply"></i><?=lg('Go Back')?></button>
    </div>
  </div>
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/custom/tracker.php?a=update" method="post">
      <dl class="horizontal">
      <?
	  $s = "SELECT * FROM $table WHERE `$key`='$id'";
	  if($r = $dx->get_row($s)){
		...
		switch($field['type']){
			...
			case 'html':
				echo '<textarea id="Edit'.$guid.'-'.$name.'" name="'.$name.'" class="form-control" rows="7">'.$data.'</textarea>';
				break;
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/custom/tracker.php?a=view')"><i class="icon-list-alt"></i><?=lg('View')?></button>
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/custom/tracker.php?a=edit')"><i class="icon-reply"></i><?=lg('Go Back')?></button>
    </div>
  </div>
  <div class="panel-body">
    <p><?
  ...
  $alert = 'Nothing update!';
	if(count($pair)>0){
		$s = "SELECT Headinc,Tracker FROM $table WHERE `$key`='$id'";
		$old = $dx->get_row($s, ARRAY_A);
		...
		if($dx->query($s)) {
			...
			// New data
			$ss = "SELECT Headinc,Tracker FROM $table WHERE `$key`='$id'";
			$new = $dx->get_row($ss, ARRAY_A);

			// Current member
			$memID = QsvMember();
			$name = GetField($dx,PREFIX_NAME.'member'.SUPFIX_NAME,"memID='$memID'",'Name');
			$email = GetField($dx,PREFIX_NAME.'member'.SUPFIX_NAME,"memID='$memID'",'Email');

			// Save changes log
			$data = "CHANGE CODE TRACKER!\r\n";
			$data .= "Member: [$memID] $name <$email>\r\n";
			$data .= "Old data: ".print_r($old,true)."\r\n";
			$data .= "New data: ".print_r($new,true)."\r\n";
			$data .= "Time: ".date('H:i:s d/m/Y')."\r\n";
			$data .= "Client IP: ".ClientIP()."\r\n";

      $log =  UPLOAD_PATH.'/logs/tracker-'.date('Y-m-d').'.log';
    ...
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/custom/tracker.php?a=view')"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
<? }?>
