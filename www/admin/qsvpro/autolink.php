...
$d = str_replace('/admin/qsvpro/autolink.php','',$f);
...
$title = lg('Auto Link');
$table = PREFIX_NAME.'seo_autolink'.SUPFIX_NAME;
$key = 'autoID';
$fields = array(
	"Keyword" => array(
		"title" => lg("Keyword"),
...
// Default view data
if(empty($_GET['a'])){
	echo '<script>Load("/admin/qsvpro/autolink.php?a=list")</script>';
	exit;
}
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      ...
      <button type="button" class="btn btn-default btn-xs" onClick="Import()"><i class="icon-download-alt"></i><?=lg('Import Excel')?></button>
      ...
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/qsvpro/autolink.php?a=add')"><i class="icon-plus"></i><?=lg('Add')?></button>
      <? }?> </div>
  </div>
  <form class="filter" id="Filter<?=$guid?>" action="/admin/qsvpro/autolink.php?a=list" method="post">
    ...
    <table class="table table-striped table-hover">
      ...
      if($rs = $dx->get_results($s)){
        ...
        <tr>
		  ...
		  ?></td>
          <? }?>
          <td><div class="action"><a href="#view" onClick="return Load('/admin/qsvpro/autolink.php?a=view',{'id':<?=$id?>,'pg':<?=$page?>})" title="View"><i class="icon-eye-open"></i> Xem</a> <? if(ValidKey($keys['edit'])){?><a href="#edit" onClick="return Load('/admin/qsvpro/autolink.php?a=edit',{'id':<?=$id?>,'pg':<?=$page?>})" title="Edit"><i class="icon-edit"></i> Sửa</a><? }?> <? if(ValidKey($keys['del'])){?><a href="#del" onClick="return Delete('/admin/qsvpro/autolink.php?a=del',{'id':<?=$id?>,'pg':<?=$page?>})" title="Delete"><i class="icon-trash"></i> Xóa</a><? }?></div></td>
        </tr>
        <?
		}
	  }
  ...
  LoadPage($total,$page,'/admin/qsvpro/autolink.php?a=list',$uf);
  ?>
</div>
<script>
BuildForm('Filter<?=$guid?>');

function Export(){
	var url = '/admin/qsvpro/autolink.php?a=export';
	window.open(url,'_blank');
	return false;
}

function Import(){
	Load('/admin/qsvpro/autolink.php?a=import');
	return false;
}
</script>
<?
...
<table width="100%" border="1" cellpadding="5" cellspacing="0" bordercolor="#cccccc" style="border-collapse:collapse">
  ...
  <tbody>
  <?
  $s = "SELECT * FROM ".PREFIX_NAME.'seo_autolink'.SUPFIX_NAME."
  		  WHERE Active='1' ORDER BY Keyword";
  $no = 0;
  if($rs = $dx->get_results($s)){
	foreach($rs as $r){?>
    <tr>
      <td><?=stripslashes($r->Keyword)?></td>
      <td><?=$r->Link?></td>
	    <td><?=$r->NgayCN?></td>
    </tr>
    <?
...
<?
  $name = sprintf('autolink_%06d',date('dHi'));
  $html=ob_get_clean();
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/qsvpro/autolink.php?a=list')"><i class="icon-list-alt"></i><?=lg('List')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
    </div>
  </div>
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/autolink.php?a=process" method="post">
      <dl class="horizontal">
		<dt>
          <label for="Add<?=$guid?>-Guide">Hướng dẫn</label>
        </dt>
        <dd>
          <div id="Add<?=$guid?>-Guide" style="padding-top:8px;min-height:37px">
          	<p>Upload file Excel theo định dạng .xlsx với nội dung theo mẫu phía dưới!</p>
          </div>
        </dd>
        <dt>
          <label for="Add<?=$guid?>-Sample">Nội dung mẫu</label>
        </dt>
        <dd>
	      <table id="Add<?=$guid?>-Sample" border="1" cellpadding="5" style="border-collapse:collapse;margin-bottom:20px">
	      	<tr>
	      	  <td><b>Keyword</b></td>
	      	  <td><b>Link</b></td>
	      	</tr>
        ...
        <dt>
          <label for="Add<?=$guid?>-Excel">Upload file Excel</label>
        </dt>
        <dd>
          <div id="Add<?=$guid?>-Excel" class="plupload clearfix" data-type="file">
            <div class="pull-left"> <a id="Pick<?=$guid?>-Excel" href="#pick" class="btn btn-info btn-sm">Upload file</a>
        ...
        <dd>
          <button type="button" class="btn btn-primary btn-sm" onClick="SubmitForm('Form<?=$guid?>')">Xác nhận</button>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <p><?
	$path = RealFile($_POST['Excel']);
	//$path = UPLOAD_PATH.'/autolink.xlsx';
	...
	if(file_exists($path)) {
		...
		echo '<h4>Import AutoLink</h4>';
		echo '<ol>';
		$max = $sheet->getHighestRow();
		for($r = 2; $r <= $max; $r++ ) {
		  $data = GetListCell($sheet, 'A'.$r, 'B'.$r);
		...
		$alert = '<h4>Import successful!</h4>';
	}
	else $alert = '<h4>Insert faulty!</h4>';
	
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/autolink.php?a=list')"><i class="icon-play"></i> Xem danh sách</button>
  </div>
</div>
<?
}
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
	...
	?>
      <dd>
        <button type="button" class="btn btn-info btn-sm" onClick="Load('/admin/qsvpro/autolink.php?a=list',{'page':<?=$pg?>})"><?=lg('Go back')?></button>
      </dd>
    </dl>
  </div>
</div>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/autolink.php?a=insert" method="post">
      <dl class="horizontal">
        <?
		foreach($fields as $name=>$field){
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/autolink.php?a=update" method="post">
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/autolink.php?a=list')"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/autolink.php?a=list',{'page':<?=$pg?>})"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
  // Auto refresh if success
  if($dx->rows_affected>0){
	echo '<script>Load("/admin/qsvpro/autolink.php?a=list",{"page":'.$pg.'})</script>';
  }
}
