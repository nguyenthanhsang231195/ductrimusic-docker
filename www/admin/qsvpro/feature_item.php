...
$d = str_replace('/admin/qsvpro/feature_item.php','',$f);
...
if(MULTI_LANGUAGE && lc()!=DEFAULT_LANGUAGE){
  echo '<script>Load("/admin/qsvpro/feature_item_lg.php")</script>';
  exit;
}
...
$table = PREFIX_NAME.'feature_item'.SUPFIX_NAME;
$key = 'fitID';
$fields = array(
  "feaID" => array(
		"title" => lg("Feature"),
		...
		"list" => true,
		"defaultValue" => "0",
		"options" => "/process/opt.php?t=feature&d=Ten&o=Ten",
		"sorting" => true,
		"type" => "list",
		"visibility" => "visible",
		"width" => "30%"
...
// Default view data
if(empty($_GET['a'])){
	echo '<script>Load("/admin/qsvpro/feature_item.php?a=list")</script>';
	exit;
}
...
<div class="panel panel-default magbtmz">
  ...
  <form class="filter" id="Filter<?=$guid?>" action="/admin/qsvpro/feature_item.php?a=list" method="post">
    ...
    <table class="table table-striped table-hover">
      ...
      if($rs = $dx->get_results($s)){
        ...
        <tr>
		  ...
		  ?></td>
          <? }?>
          <td><div class="action"><a href="#view" onClick="return Load('/admin/qsvpro/feature_item.php?a=view',{'id':<?=$id?>,'pg':<?=$page?>})" title="View"><i class="icon-eye-open"></i> <?=lg('View')?></a> <? if(ValidKey($keys['edit'])){?><a href="#edit" onClick="return Load('/admin/qsvpro/feature_item.php?a=edit',{'id':<?=$id?>,'pg':<?=$page?>})" title="Edit"><i class="icon-edit"></i> <?=lg('Modify')?></a><? }?></div></td>
        </tr>
        <?
		  }
	  }
  ...
  LoadPage($total,$page,'/admin/qsvpro/feature_item.php?a=list',$uf);
  ?> </div>
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/qsvpro/feature_item.php?a=list')"><i class="icon-list-alt"></i><?=lg('List')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
    </div>
  </div>
  <div class="panel-body">
	...
	?>
      <dd>
        <button type="button" class="btn btn-info btn-sm" onClick="Load('/admin/qsvpro/feature_item.php?a=list',{'page':<?=$pg?>})"><?=lg('Go back')?></button>
      </dd>
    </dl>
  </div>
</div>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/feature_item.php?a=update" method="post">
...
<div class="panel panel-default magbtmz">
  ...
  $alert = 'Nothing update!';
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/feature_item.php?a=list',{'page':<?=$pg?>})"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
  // Auto refresh if success
  if($dx->rows_affected>0){
	echo '<script>Load("/admin/qsvpro/feature_item.php?a=list",{"page":'.$pg.'})</script>';
  }
}
?>