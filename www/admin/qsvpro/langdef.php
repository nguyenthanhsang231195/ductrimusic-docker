...
$d = str_replace('/admin/qsvpro/langdef.php','',$f);
...
$title = lg('Translate text');
$table = PREFIX_NAME.'langdef'.SUPFIX_NAME;
$key = 'txtID';
$fields = array(
	"key" => array(
		"title" => lg("Original word"),
		"create" => true,
		"edit" => true,
		"list" => true,
		"sorting" => true,
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      ...
      <? if(DEVELOPMENT){?>
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/qsvpro/langdef.php?a=add')"><i class="icon-plus"></i><?=lg('Add')?></button>
      <? }?>
    </div>
  </div>
    ...
    <table class="table table-striped table-hover">
      ...
      if($rs = $dx->get_results($s)){
        ...
        <tr>
          ...
          <? }?>
          <td><div class="action">
            <a href="#view" onClick="return Load('/admin/qsvpro/langdef.php?a=view',{'id':<?=$id?>})" title="View"><i class="icon-eye-open"></i> <?=lg('View')?></a>
            <a href="#edit" onClick="return Load('/admin/qsvpro/langdef.php?a=edit',{'id':<?=$id?>})" title="Edit"><i class="icon-edit"></i> <?=lg('Modify')?></a>
            <? if(DEVELOPMENT){?><a href="#del" onClick="return Delete('/admin/qsvpro/langdef.php?a=del',<?=$id?>)" title="Delete"><i class="icon-trash"></i> <?=lg('Delete')?></a>
            <? }?>
          </div></td>
        </tr>
        <?
  ...
  LoadPage($total,$page,'/admin/qsvpro/langdef.php?a=list');
  ?>
</div>
<?
}
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/qsvpro/langdef.php?a=list')"><i class="icon-list-alt"></i><?=lg('List')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
    </div>
  </div>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/langdef.php?a=insert" method="post">
      <dl class="horizontal">
        <?
		foreach($fields as $name=>$field){
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/langdef.php?a=update" method="post">
...
<div class="panel panel-default magbtmz">
  ...
  $s = "INSERT INTO $table(`".join("`,`",array_keys($val))."`) VALUES('".join("','",array_values($val))."')";
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/langdef.php?a=list')"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
  // Auto refresh if success
  if($dx->rows_affected>0){
	echo '<script>Load("/admin/qsvpro/langdef.php?a=list")</script>';
  }
}
