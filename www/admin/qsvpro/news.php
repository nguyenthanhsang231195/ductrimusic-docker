...
$d = str_replace('/admin/qsvpro/news.php','',$f);
...
$title = lg('News');
$table = PREFIX_NAME.'news'.SUPFIX_NAME;
$key = 'newsID';
$fields = array(
	"Ten" => array(
		"title" => lg("Name"),
  ...
  "ncatID" => array(
    ...
    "defaultValue" => "0",
    "type" => "combobox",
    "options" => "/process/opt.php?t=news_catalog&d=Ten&o=Thutu&p=chID&f=$lang",
    "width" => "10%"
  ),
	"Hot" => array(
		"title" => lg("Hot"),
...
// Default view data
if(empty($_GET['a'])){
	echo '<script>Load("/admin/qsvpro/news.php?a=list")</script>';
	exit;
}
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      ...
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/qsvpro/news.php?a=add')"><i class="icon-plus"></i><?=lg('Add')?></button>
      <? }?> </div>
  </div>
  <form class="filter" id="Filter<?=$guid?>" action="/admin/qsvpro/news.php?a=list" method="post">
    ...
    <table class="table table-striped table-hover">
      ...
      if($rs = $dx->get_results($s)){
        ...
        <tr>
		  ...
		  ?></td>
          <? }?>
          <td><div class="action"><a href="#view" onClick="return Load('/admin/qsvpro/news.php?a=view',{'id':<?=$id?>,'pg':<?=$page?>})" title="View"><i class="icon-eye-open"></i> <?=lg('View')?></a> <? if(ValidKey($keys['edit'])){?><a href="#edit" onClick="return Load('/admin/qsvpro/news.php?a=edit',{'id':<?=$id?>,'pg':<?=$page?>})" title="Edit"><i class="icon-edit"></i> <?=lg('Modify')?></a><? }?> <? if(ValidKey($keys['del'])){?><a href="#del" onClick="return Delete('/admin/qsvpro/news.php?a=del',{'id':<?=$id?>,'pg':<?=$page?>})" title="Delete"><i class="icon-trash"></i> <?=lg('Delete')?></a><? }?></div></td>
        </tr>
        <?
		}
	  }
  ...
  LoadPage($total,$page,'/admin/qsvpro/news.php?a=list',$uf);
  ?> </div>
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/qsvpro/news.php?a=list')"><i class="icon-list-alt"></i><?=lg('List')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
    </div>
  </div>
  <div class="panel-body">
	...
	?>
      <dd>
        <button type="button" class="btn btn-info btn-sm" onClick="Load('/admin/qsvpro/news.php?a=list',{'page':<?=$pg?>})"><?=lg('Go back')?></button>
      </dd>
    </dl>
  </div>
</div>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/news.php?a=insert" method="post">
      <dl class="horizontal">
        <?
		foreach($fields as $name=>$field){
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/news.php?a=update" method="post">
...
<div class="panel panel-default magbtmz">
  ...
  $s = "INSERT INTO $table(`".join("`,`",array_keys($val))."`) VALUES('".join("','",array_values($val))."')";
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/news.php?a=list')"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/news.php?a=list',{'page':<?=$pg?>})"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
  // Auto refresh if success
  if($dx->rows_affected>0){
	echo '<script>Load("/admin/qsvpro/news.php?a=list",{"page":'.$pg.'})</script>';
  }
}
