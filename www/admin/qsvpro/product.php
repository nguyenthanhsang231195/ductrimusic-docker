...
$d = str_replace('/admin/qsvpro/product.php','',$f);
...
if(MULTI_LANGUAGE && lc()!=DEFAULT_LANGUAGE){
	echo '<script>Load("/admin/qsvpro/product_lg.php")</script>';
	exit;
}
...
if(!empty($keys['mgr']) && ValidKey($keys['mgr'])){
	$keys['view'] = $keys['mgr'];
	$keys['add'] = $keys['mgr'];
	$keys['edit'] = $keys['mgr'];
	$keys['del'] = $keys['mgr'];
...
if(!isset($keys['price'])) $keys['price'] = $keys['all'];
...
$title = lg('Product');
$table = PREFIX_NAME.'product'.SUPFIX_NAME;
$key = 'proID';
$fields = array(
	"Ten" => array(
		"title" => lg("Name"),
  ...
  "Mausac" => array(
		...
		"list" => true,
		"defaultValue" => "",
		"options" => "/process/opt.php?t=product_color&d=Ten&o=Thutu",
		"sorting" => true,
		"type" => "list",
		"visibility" => "visible",
		"width" => "10%"
  ...
  */
  "Danhmuc" => array(
		...
		"list" => true,
		"defaultValue" => "0",
		"options" => "/process/opt.php?t=product_catalog&d=Ten&o=Thutu&p=chID",
		"sorting" => true,
		"type" => "list",
		"visibility" => "visible",
		"width" => "10%"
	...
	"Bosuutap" => array(
		...
		"list" => true,
		"defaultValue" => "",
		"options" => "/process/opt.php?t=collection&d=Ten&o=Thutu&f=$lang",
		"sorting" => true,
		"type" => "list",
		"visibility" => "visible",
		"width" => "10%"
  ...
  "Lienquan" => array(
	...
	"Status" => array(
		...
		"visibility" => "visible",
		"width" => "10%",
		"values" => array("0" => "Còn hàng","1" => "Hết hàng")
	),
	"Hot" => array(
		"title" => lg("Hot"),
		"create" => true,
...
// Quyen xem bang gia
if(!ValidKey($keys['price'])) {
  unset($fields['Filegia']);
  unset($fields['Giaban']);
  unset($fields['GiaKM']);
}
...
// Default view data
if(empty($_GET['a'])){
	echo '<script>Load("/admin/qsvpro/product.php?a=list")</script>';
	exit;
}
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      ...
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/qsvpro/product.php?a=add')"><i class="icon-plus"></i><?=lg('Add')?></button>
      <? }?> </div>
  </div>
  <form class="filter" id="Filter<?=$guid?>" action="/admin/qsvpro/product.php?a=list" method="post">
    ...
    <table class="table table-striped table-hover">
      ...
      if($rs = $dx->get_results($s)){
        ...
        <tr>
		  ...
		  ?></td>
          <? }?>
          <td><div class="action"><a href="#view" onClick="return Load('/admin/qsvpro/product.php?a=view',{'id':<?=$id?>,'pg':<?=$page?>})" title="View"><i class="icon-eye-open"></i> <?=lg('View')?></a><br> <? if(ValidKey($keys['edit'])){?><a href="#edit" onClick="return Load('/admin/qsvpro/product.php?a=edit',{'id':<?=$id?>,'pg':<?=$page?>})" title="Edit"><i class="icon-edit"></i> <?=lg('Modify')?></a><br><? }?> <? if(ValidKey($keys['del'])){?><a href="#del" onClick="return Delete('/admin/qsvpro/product.php?a=del',{'id':<?=$id?>,'pg':<?=$page?>})" title="Delete"><i class="icon-trash"></i> <?=lg('Delete')?></a><? }?></div></td>
        </tr>
        <?
		}
	  }
  ...
  LoadPage($total,$page,'/admin/qsvpro/product.php?a=list',$uf);
  ?> </div>
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/qsvpro/product.php?a=list')"><i class="icon-list-alt"></i><?=lg('List')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
    </div>
  </div>
  <div class="panel-body">
	...
	?>
      <dd>
        <button type="button" class="btn btn-info btn-sm" onClick="Load('/admin/qsvpro/product.php?a=list',{'page':<?=$pg?>})"><?=lg('Go back')?></button>
      </dd>
    </dl>
  </div>
</div>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/product.php?a=insert" method="post">
      <dl class="horizontal">
        <?
		foreach($fields as $name=>$field){
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/product.php?a=update" method="post">
...
<div class="panel panel-default magbtmz">
  ...
  // Quyen them bang gia
  if(ValidKey($keys['price'])) $val['PriceSync'] = 0;
  ...
  $s = "INSERT INTO $table(`".join("`,`",array_keys($val))."`) VALUES('".join("','",array_values($val))."')";
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/product.php?a=list')"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
...
<div class="panel panel-default magbtmz">
  ...
  $alert = 'Nothing update!';
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/product.php?a=list',{'page':<?=$pg?>})"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
  // Auto refresh if success
  if($dx->rows_affected>0){
	echo '<script>Load("/admin/qsvpro/product.php?a=list",{"page":'.$pg.'})</script>';
  }
}
  ...
  <div class="panel-body">
    <p><?
      ...
      if($dx->query($s)) {
        ...
        DeleteField($dx,PREFIX_NAME.'product_lg'.SUPFIX_NAME,"lgID='$id'");
