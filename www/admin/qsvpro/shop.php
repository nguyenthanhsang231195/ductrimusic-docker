...
$d = str_replace('/admin/qsvpro/shop.php','',$f);
...
$title = lg('Shop');
$table = PREFIX_NAME.'shop'.SUPFIX_NAME;
$key = 'shID';
$fields = array(
  "Ten" => array(
		"title" => lg("Name"),
  ...
  "Thutu" => array(
	...
	"Loai" => array(
		...
		"visibility" => "visible",
		"width" => "10%",
		"values" => array("0" => "Silde","1" => "Địa chỉ Showroom")
	),
	"Active" => array(
		"title" => lg("Active"),
		"create" => true,
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      ...
      <button type="button" class="btn btn-warning btn-xs" onClick="Load('/admin/qsvpro/shop.php?a=add')"><i class="icon-plus"></i><?=lg('Add')?></button>
    </div>
  </div>
    ...
    <table class="table table-striped table-hover">
      ...
      if($rs = $dx->get_results($s)){
        ...
        <tr>
		  ...
		  ?></td>
          <? }?>
          <td><div class="action"><a href="#view" onClick="return Load('/admin/qsvpro/shop.php?a=view',{'id':<?=$id?>})" title="View"><i class="icon-eye-open"></i></a> <a href="#edit" onClick="return Load('/admin/qsvpro/shop.php?a=edit',{'id':<?=$id?>})" title="Edit"><i class="icon-edit"></i></a> <a href="#del" onClick="return Delete('/admin/qsvpro/shop.php?a=del',<?=$id?>)" title="Delete"><i class="icon-trash"></i></a></div></td>
        </tr>
        <?
        }
	  }
  ...
  LoadPage($total,$page,'/admin/qsvpro/shop.php?a=list');
  ?>
</div>
<?
}
...
<div class="panel panel-default magbtmz">
  <div class="panel-heading"><?=$title?>
    <div class="control">
      <button type="button" class="btn btn-default btn-xs" onClick="Load('/admin/qsvpro/shop.php?a=list')"><i class="icon-list-alt"></i><?=lg('List')?></button>
      <button type="button" class="btn btn-success btn-xs" onClick="Refresh()"><i class="icon-refresh"></i><?=lg('Refresh')?></button>
    </div>
  </div>
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/shop.php?a=insert" method="post">
      <dl class="horizontal">
        <?
		foreach($fields as $name=>$field){
...
<div class="panel panel-default magbtmz">
  ...
  <div class="panel-body">
    <form id="Form<?=$guid?>" class="custom" action="/admin/qsvpro/shop.php?a=update" method="post">
...
<div class="panel panel-default magbtmz">
  ...
  $s = "INSERT INTO $table(`".join("`,`",array_keys($val))."`) VALUES('".join("','",array_values($val))."')";
	...
	echo $alert;
    ?></p>
    <button type="button" class="btn btn-default btn-sm" onClick="Load('/admin/qsvpro/shop.php?a=list')"><i class="icon-play"></i> <?=lg('Continue')?></button>
  </div>
</div>
  <?
  // Auto refresh if success
  if($dx->rows_affected>0){
	echo '<script>Load("/admin/qsvpro/shop.php?a=list")</script>';
  }
}
