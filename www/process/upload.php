...
if(!$login){
	die('{"jsonrpc" : "2.0", "error" : {"code": 99, "message": "Access denied!"}}');
}
...
if (!$chunks || $chunk == $chunks - 1) $tmpFile = "{$filePath}.part";	
else $tmpFile = $filePath;
$filePath = $targetDir . '/' . SaveName($fileName);
rename($tmpFile, $filePath);

$display = VIEW_PATH.'/'.GetDisplay($filePath);
die('{"jsonrpc" : "2.0", "result" : "'.$display.'"}');
